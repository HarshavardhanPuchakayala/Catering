  ...
  const uplodeImage = (e) =>{
    ...
    uploadTask.on(
      ...
      (error) => {
        console.log(error)
        setFields(true)
        setMsg("Error while uploding : Try Again 🥵")
    ...
    <div className="w-full min-h-screen flex items-center justify-center">
        <div className="w-[90%] md:w-[75%] border border-gray-300 rounded-lg p-4 flex flex-col items-center justify-center gap-4">
          ...
          <div className="w-full">
            ...
            className='outline-none w-full text-base border-b-2 border-gray-200 p-2 rounded-md cursor-pointer bg-primary my-3'>
              ...
              {categories && categories.map((item)=>(
                ...
                className='text-base border-0 outline-none capitalize
                 bg-white text-headingColor' value={item.urlParamName}>
                  {item.name}
                 </option>
              ))}
            </select>
            ...
            flex-col border-2 border-dotted border-gray-300 w-full
              h-420 cursor-pointer rounded-lg">
             {isLoading ? (<Loader />) :( <>
             {!imageAsset ? (<>
             <label className='w-full h-full flex flex-col items-center justify-center cursor-pointer '>
              <div className="w-full h-full flex flex-col items-center justify-center gap-2">
                ...
                <div className='relative h-full'>
                  <img src={imageAsset} alt="uplode image" className='w-full h-full object-cover'/>
                  ...
                  ease-in-out' onClick={deleteImage}><MdDelete className='text-white'/></button></div></>)}
             </>
             )}
             </div>
         ...
         <div className="flex items-center w-full">
          ...
          px-12 rounded-lg text-lg text-white font-semibold'
          onClick={saveDetails}>
              save
          </button>
         </div>
